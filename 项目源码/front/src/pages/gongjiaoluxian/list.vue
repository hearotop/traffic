...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 7%","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"center","height":"auto"}'>
      ...
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>起始点</div>
        <el-select v-model="formSearch.qishidian" placeholder="请选择起始点" :clearable="true">
          <el-option v-for="(item, index) in qishidianOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>途径地</div>
        <el-input v-model="formSearch.tujingdi" placeholder="途径地" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>终止点</div>
        <el-select v-model="formSearch.zhongzhidian" placeholder="请选择终止点" :clearable="true">
          <el-option v-for="(item, index) in zhongzhidianOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('gongjiaoluxian','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 10px","margin":"0 4px 0 0","color":"#fff","minWidth":"90px","outline":"none","borderRadius":"0","background":"#f50000","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/gongjiaoluxianAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"calc(93% - 200px)","padding":"20px 7%","margin":"20px 0 10px","minWidth":"900px","background":"none","flex":"1"}'>
		...
		<div class="list2 index-pv1" :style='{"padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"10px","margin":"0 0 20px","flexWrap":"wrap","background":"none","display":"flex","width":"49%","fontSize":"0","position":"relative","justifyContent":"space-between","height":"240px"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"48%","padding":"4px","objectFit":"cover","background":"url(http://codegen.caihongy.cn/20230201/e692aea55f98482aaa7f0290e1888901.png) no-repeat center top / 100% 100%","display":"inline-block","height":"100%"}' v-if="item.xianlutupian && item.xianlutupian.substr(0,4)=='http'" :src="item.xianlutupian" class="image" />
				<img :style='{"width":"48%","padding":"4px","objectFit":"cover","background":"url(http://codegen.caihongy.cn/20230201/e692aea55f98482aaa7f0290e1888901.png) no-repeat center top / 100% 100%","display":"inline-block","height":"100%"}' v-else :src="baseUrl + (item.xianlutupian?item.xianlutupian.split(',')[0]:'')" class="image" />
				<div class="item-info" :style='{"width":"48%","padding":"10px","overflow":"hidden","display":"inline-block","height":"100%"}'>
					...
					<div :style='{"border":"0 dashed #eee","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"center","background":"linear-gradient(90deg, rgba(255,255,255,1) 0%, rgba(238,238,238,1) 50%, rgba(255,255,255,1) 100%)","borderWidth":"1px 0","lineHeight":"40px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">票价:{{item.piaojia}}</div>
					<div :style='{"border":"0 dashed #eee","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"center","background":"linear-gradient(90deg, rgba(255,255,255,1) 0%, rgba(238,238,238,1) 50%, rgba(255,255,255,1) 100%)","borderWidth":"1px 0","lineHeight":"40px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.chepaihao}}</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/zhandianxinxi/zhandianmingcheng').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.qishidian != '') searchWhere.qishidian = this.formSearch.qishidian;
        if (this.formSearch.tujingdi != '') searchWhere.tujingdi = '%' + this.formSearch.tujingdi + '%';
        if (this.formSearch.zhongzhidian != '') searchWhere.zhongzhidian = this.formSearch.zhongzhidian;
        this.$http.get('gongjiaoluxian/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/gongjiaoluxianDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
