	...
	<div class="addEdit-block" :style='{"padding":"30px"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.weihufengmian" label="维护封面" prop="weihufengmian">
					<file-upload
						tip="点击上传维护封面"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.weihufengmian?ruleForm.weihufengmian:''"
						@change="weihufengmianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.weihufengmian" label="维护封面" prop="weihufengmian">
					<img v-if="ruleForm.weihufengmian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.weihufengmian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.weihufengmian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="维护内容" prop="weihuneirong">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.weihuneirong" label="维护内容" prop="weihuneirong">
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='weihufengmian'){
							this.ruleForm.weihufengmian = obj[o];
							this.ro.weihufengmian = true;
							continue;
						}
						if(o=='kaishishijian'){
	...
	if(this.ruleForm.weihufengmian!=null) {
		this.ruleForm.weihufengmian = this.ruleForm.weihufengmian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xianluweihu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xianluweihuCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    weihufengmianUploadChange(fileUrls) {
	    this.ruleForm.weihufengmian = fileUrls;
    },
  }
};
</script>
